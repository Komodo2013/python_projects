...
def initial_config():
    ...
    if defaults == False:
        ...
        if advance_setup == True:
            ...
            print(
                "Enter any Label IDs that you would like assigned to the tasks, separated by comma (pull using todoist_labels.py)")
...
def transfer_assignments_to_todoist():
    for assignment in assignments:
        ...
        for task in todoist_tasks:
            if task['content'] == ('[' + assignment['name'] + '](' + assignment['html_url'] + ')' + ' Due') and \
                ...
                if (task['due'] and task['due']['date'] != assignment['due_at']):
                    is_synced = False
                    item = task
                    print("Updating assignment due date: " + course_name + ": " + assignment['name'] + " to " + str(
                        assignment['due_at']))
                    break
                else:
            ...
            if assignment['due_at'] == None and config['sync_no_due_date_assignments'] == False:
                ...
                is_added = True
                break
            if assignment['unlock_at'] != None and config['sync_locked_assignments'] == False and assignment[
                'unlock_at'] > (datetime.datetime.now() + datetime.timedelta(days=1)).isoformat():
                print(
                    "Ignoring assignment that is not yet unlocked: " + course_name + ": " + assignment['name'] + ": " +
                    assignment['lock_explanation'])
                is_added = True
                break
            if assignment['locked_for_user'] == True and assignment['unlock_at'] == None and config[
        ...
        if not is_added:
            if assignment['submission']['submitted_at'] == None or assignment['submission'][
                'workflow_state'] == "unsubmitted" or assignment['submission']['attempt'] == None:
